...
try{
    ...
    if($PSCmdlet.ParameterSetName  -eq "Snapshot"){
        ...
        $Script:floppy = Get-FloppyDrive -Server $Script:vmServer -Snapshot $snap -ErrorAction Stop
    }
    elseif($PSCmdlet.ParameterSetName  -eq "Template"){
        $temp = Get-Template -Server $Script:vmServer -Name $TemplateName -ErrorAction Stop
        $Script:floppy = Get-FloppyDrive -Server $Script:vmServer -Template $temp -ErrorAction Stop
    }
    else {
        $vm = Get-VM -Server $Script:vmServer -Name $VMName -ErrorAction Stop        
        $Script:floppy = Get-FloppyDrive -Server $Script:vmServer -VM $vm -ErrorAction Stop
    }
