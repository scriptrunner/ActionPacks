...
try{
    ...
    if($null -ne $Script:User){
        ...
        if(-not $Simulate){
            if($PSCmdlet.ParameterSetName  -eq "Remote Jumphost"){
                $Script:User = Set-ADUser -Credential $DomainAccount -Server $Script:Domain.PDCEmulator -AuthType $AuthType -Instance $Script:User -PassThru -Confirm:$false -ErrorAction Stop 
            }
            else {
                $Script:User = Set-ADUser -Server $Script:Domain.PDCEmulator -AuthType $AuthType -Instance $Script:User -PassThru -Confirm:$false -ErrorAction Stop 
            }   
        }
