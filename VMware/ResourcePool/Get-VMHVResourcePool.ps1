...
try{
    ...
    if($PSCmdlet.ParameterSetName  -eq "byID"){
        $Script:Output = Get-ResourcePool -Server $Script:vmServer -ID $ID -NoRecursion:$NoRecursion -ErrorAction Stop | Select-Object $Properties.Split(",")
    }
    elseif($PSCmdlet.ParameterSetName  -eq "byName"){
        if([System.String]::IsNullOrWhiteSpace($Name) -eq $true){
            $Script:Output = Get-ResourcePool -Server $Script:vmServer -NoRecursion:$NoRecursion -ErrorAction Stop | Select-Object $Properties.Split(",")
        }
        else{
            $Script:Output = Get-ResourcePool -Server $Script:vmServer -Name $Name -NoRecursion:$NoRecursion -ErrorAction Stop | Select-Object $Properties.Split(",")
        }        
    }
    elseif($PSCmdlet.ParameterSetName  -eq "byVM"){
        $Script:Output = Get-ResourcePool -Server $Script:vmServer -VM $VM  -NoRecursion:$NoRecursion -ErrorAction Stop | Select-Object $Properties.Split(",")
    }
