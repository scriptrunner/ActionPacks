...
foreach($id in $RoleIds){
    ...
    if($null -ne $UserIds){
        ...
        foreach($itm in $UserIds){
            ...
            try{
                Remove-MsolRoleMember -TenantId $TenantId -RoleObjectId $role.ObjectID -RoleMemberObjectId $usr.ObjectID -RoleMemberType 'User'
    ...
    if($null -ne $ServicePrincipalIds){
        ...
        foreach($itm in $ServicePrincipalIds){
            ...
            try{
                Remove-MsolRoleMember -TenantId $TenantId -RoleObjectId $role.ObjectID -RoleMemberObjectId $srv.ObjectID -RoleMemberType 'ServicePrincipal'
