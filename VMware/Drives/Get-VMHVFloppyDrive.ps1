...
.Parameter DriveName
    Specifies the name of the floppy drive you want to retrieve, is the parameter empty all floppy drives retrieved
#>
...
try{
    ...
    if($PSCmdlet.ParameterSetName  -eq "Snapshot"){
        ...
        $Script:Output = Get-FloppyDrive -Server $Script:vmServer -Snapshot $snap -Name $DriveName -ErrorAction Stop | Select-Object *
    }
    elseif($PSCmdlet.ParameterSetName  -eq "Template"){
        $temp = Get-Template -Server $Script:vmServer -Name $TemplateName -ErrorAction Stop
        $Script:Output = Get-FloppyDrive -Server $Script:vmServer -Template $temp -Name $DriveName -ErrorAction Stop | Select-Object *
    }
    else {
        $vm = Get-VM -Server $Script:vmServer -Name $VMName -ErrorAction Stop        
        $Script:Output = Get-FloppyDrive -Server $Script:vmServer -VM $vm -Name $DriveName -ErrorAction Stop | Select-Object *
    }
