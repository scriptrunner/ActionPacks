...
try{
    ...
    if($PSCmdlet.ParameterSetName  -eq "Snapshot"){
        ...
        $Script:Output = Get-NetworkAdapter -Server $Script:vmServer -Snapshot $snap -Name $AdapterName -ErrorAction Stop | Select-Object *
    }
    elseif($PSCmdlet.ParameterSetName  -eq "Template"){
        $temp = Get-Template -Server $Script:vmServer -Name $TemplateName -ErrorAction Stop
        $Script:Output = Get-NetworkAdapter -Server $Script:vmServer -Template $temp -Name $AdapterName -ErrorAction Stop | Select-Object *
    }
    else {
        $vm = Get-VM -Server $Script:vmServer -Name $VMName -ErrorAction Stop        
        $Script:Output = Get-NetworkAdapter -Server $Script:vmServer -VM $vm -Name $AdapterName -ErrorAction Stop | Select-Object *
    }
